...
public final class Base64Encoder {
	...
	private static final String BASE64__URL_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";
	...
	public static String encodeUrl(byte[] value) {
		...
		for (int i = 0; i < value.length; i++) {
			...
			if (bufferIndex == 3 || i == (value.length - 1)) {
				...
				// Make a third character
				base64CharIndex = ((buffer[1] & 0x0F) << 2) | (buffer[2] >>> 6);
				output[outputIndex++] = bufferIndex > 1 ? BASE64__URL_CHARS.charAt(base64CharIndex) : BASE64_PADDING;

				// Make a fourth character
				base64CharIndex = buffer[2] & 0x3F;
				output[outputIndex++] = bufferIndex > 2 ? BASE64__URL_CHARS.charAt(base64CharIndex) : BASE64_PADDING;
